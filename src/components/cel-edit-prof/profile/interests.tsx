...
const CelInterestsInfo: React.FC<any> = (props) => {
    const { data, updateCel, refetch } = props

    const [edit, setEdit] = useState(false)

    const [tempInterests, setTempInterests] = useState({
        defaultInterests: data?.getCelebrityById?.interests,
        selectedInterests: data?.getCelebrityById?.interests
    })
    ...
    return (
        <div className="flex flex-col py-[30px] px-[60px] shadow-[0px_0px_9px_3px_rgba(0,0,0,0.1)] rounded-2xl mt-[40px]" id='interests'>
            ...
            <span>Find common ground with fans and business by adding interests to your profile.</span>
            ...
            {edit && <FanInterests setEdit={setEdit} tempInterests={tempInterests} id={data?.getCelebrityById?.id}
            updateFan={updateCel} refetch={refetch} setTempInterests={setTempInterests} celebrity={true} />}
        </div>
    )
}

export default CelInterestsInfo