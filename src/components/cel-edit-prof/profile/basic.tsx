...
const CelBasicInfo: React.FC<any> = (props) => {
    const { data, updateCel, updateUser, refetch } = props

    const [defaultValues, setDefaultValues] = useState({
        name: data?.getCelebrityById?.name,
        email: data?.getCelebrityById?.email,
        location: data?.getCelebrityById?.location,
        profilePic: data?.getCelebrityById?.associatedUser?.profilePic,
        birthYear: {
            startDate: data?.getCelebrityById?.birthYear,
            endDate: data?.getCelebrityById?.birthYear
        }
    })
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (basicInfo.profilePic !== defaultValues.profilePic) {
            updateUser({
                variables: {
                    user: {
                        id: data?.getCelebrityById?.userId,
                        name: basicInfo.name,
                        email: basicInfo.email,
        ...
        <div className="flex flex-col py-[30px] px-[60px] shadow-[0px_0px_9px_3px_rgba(0,0,0,0.1)] rounded-2xl" id='basic-info'>
            ...
            <div className="flex flex-row mt-[25px] gap-x-[60px]">
                <div className="flex flex-col items-center">
                    <img className="w-[220px] h-[240px] rounded-xl border border-gray-300 object-cover object-top"
